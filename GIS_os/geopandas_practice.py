...
def point_converter(shapefile):

    '''
    returns a converted shapefile that doesn't contain non-UTF characters
    '''
...
def pljv_clip(windlyr,pljvboundlyr):

    '''
    returns wind GeoDataFrame clipped to within input pljvboundary
    exports shapefile that will be called in next function
    '''

    wind = gpd.read_file(windlyr)
    pljvbound = gpd.read_file(pljvboundlyr)
    wind.crs = {'init': 'epsg:4326'} #crs of original .shp
    pljvbound.crs = {'init': 'epsg:26914'} #crs of original .shp
    ...
    windsubset.to_file(os.path.join(outDir, 'wind_clip.shp'))
    return windsubset

def convexhulls(windsubset):

    '''
    Buffers points, dissolves buffers, assigns unique ids,and...
    Returns convex hull GeoDataFrame, writes to file
    '''

    #Format for assigning buffers to geometry of a manipulable geodataframe
    windsubset = gpd.read_file(os.path.join(outDir, 'wind_clip.shp'))
    windsubset.crs = {'init': 'epsg:32614'} #previously projected to this, just redefining again
    windbuff=windsubset
    ...
    hulls_gdf.to_file(os.path.join(outDir,'conv_hulls.shp'))
    return hulls_gdf

def affected_playas(hulls_lyr,playa_lyr):

    '''
    returns GeoDataFrame of playas with field 'knock_date' updated to date of wind dataset
    release, if playa had not been previously affected, writes _copy of original layer
    '''

    playas = gpd.read_file(playa_lyr)
    hulls =gpd.read_file(hulls_lyr)
    playas.crs = {'init': 'epsg:26914'} #crs of original .shp
    playas = playas.to_crs({'init': 'epsg:32614'})
    hulls.crs = {'init': 'epsg:32614'}
    ...
    polygons = hulls.geometry
    polygons = hulls.loc[hulls['geometry'].geom_type=='Polygon'] #this works officially to trim lines and points
    ...
    playas['knock_date'][aff_playas]=wind_name[-11:] #way to selet only affected playas. seems to work.

    #import

    playas.to_file(os.path.join(outDir,playas_name+'_copy.shp'))
    return playas
